...
def main():
    ...
    source_root = os.path.abspath(os.path.join(project_root, 'source'))
    jsb_root = os.path.abspath(os.path.join(project_root, 'out'))
    cxx_generator_root = project_root

    # save config to file
    config = ConfigParser.ConfigParser()
    config.set('DEFAULT', 'androidndkdir', ndk_root )
    config.set('DEFAULT', 'clangllvmdir', llvm_path)
    config.set('DEFAULT', 'sourcedir', source_root)
    ...
    try:
        ...
        tojs_root = '%s/config' % project_root
        output_dir = '%s/out' % project_root
        ...
        #             }
        cmd_args = {'three.ini' : ('three', 'three')}
        target = 'idl'
