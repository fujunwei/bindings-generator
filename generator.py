...
class NativeType(object):
    ...
    def to_webcore_native(self, generator):
        ...
        if self_name.find("shared_ptr") != -1:
            self_name = self_name.replace("std::", "").replace("shared_ptr<", "").replace(">", "")
            self_name = self_name.replace("three::", "")
        if self_name in generator.idl_classes:
            if not self.is_pointer:
                return self_name + '*'
        return self_name
...
class NativeFunction(object):
    ...
    def native_type_to_idl(self, type, is_need_dynamic_wrap=True):
        ...
        if is_need_dynamic_wrap and self.generator.in_listed_extend_classed(ret_type):
            if type.is_pointer:
                return 'any'
        if ret_type.find("shared_ptr") != -1:
            ret_type = ret_type.replace("shared_ptr<", "").replace(">", "")
            ret_type = ret_type.replace("three::", "")
        return ret_type
    ...
    def native_type_to_idl(self, type, is_need_dynamic_wrap=True):
        ...
        if is_need_dynamic_wrap and self.generator.in_listed_extend_classed(ret_type):
            if type.is_pointer:
                return 'any'
        if ret_type.find("shared_ptr") != -1:
            ret_type.replace("shared_ptr<", "").replace(">", "")
        return ret_type
    ...
    def _process_node(self, cursor):
        ...
        if cursor.kind == cindex.CursorKind.CXX_BASE_SPECIFIER:
            ...
            if self._current_visibility == cindex.AccessSpecifierKind.PUBLIC and not cursor.type.is_function_variadic():
                ...
                if "operator" in registration_name:
                    return False
...
class Generator(object):
    ...
    def extend_from_ccobj(self, class_name):
        """
        returns True if the class is in the list of required classes and it's not in the skip list
        """
        return False
        ...
        class_name = class_name.replace("*","").replace("const ", "")
        if class_name in self.idl_classes:
            if class_name == "ccFontDefinition":
                return False
            return True
        return False
        """
