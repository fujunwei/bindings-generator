...
Uniform::Uniform( )
  : type( THREE::INVALID_UNIFORM ), value( ) { 
      // console().log() << "Uniform 1 :" << &value <<" v.obj:" << value.object << " typeid:" << value.policy->type().name() << " type:" << type;
  }

Uniform::Uniform( THREE::UniformType type, any value /*= any()*/ )
  : type( type ), value( std::move( value ) ) {
      console().log() << "Uniform 2 :" << &value <<" v.obj:" << value.object << " typeid:" << value.policy->type().name() << " type:" << type;
}

Uniform::Uniform( Uniform&& other )
  : type( THREE::INVALID_UNIFORM ) {
   swap( other ); 
      // console().log() << "Uniform 3 :" << &value <<" v.obj:" << value.object << " typeid:" << value.policy->type().name() << " type:" << type;
 }

Uniform::Uniform( const Uniform& other )
  : type( other.type ), value( other.value ) { 
      // console().log() << "Uniform 4 :" << &value <<" v.obj:" << value.object << " typeid:" << value.policy->type().name() << " type:" << type;
  }
...
Uniform& Uniform::swap( Uniform& other ) {
  ...
  // console().log() << "Uniform 5 :" << &value <<" v.obj:" << value.object << " typeid:" << value.policy->type().name() << " type:" << type;
  return *this;
}
...
void loadv( const F& f, int location, const any& value, int stride = 1 ) {
  ...
  console().log() << "====";
  console().log() << &value;
  console().log() <<"  V: " << typeid(v).name()  <<"  T: " << typeid(pv).name();
  console().log() << " typeid:" << value.policy->type().name() << " object:" << value.object;
  console().log() << "v.size:" << v.size() << " stride:" << stride << " &v:" << &value << " pv:" << pv;
  console().log() << "====";
